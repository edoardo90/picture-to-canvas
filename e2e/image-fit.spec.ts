...
// AC-1: toggle has aria-pressed=true initially (Fit is the default mode)
test('Fit toggle has aria-pressed="true" by default', async ({ page }) => {
  await page.goto('/')
...
// AC-1: clicking the toggle from Fit mode switches to Stretch (aria-pressed=false)
test('clicking Fit toggle from Fit mode sets aria-pressed to false', async ({ page }) => {
  await loadImage(page)
...
// AC-1: pressing F key toggles fit mode (starts in Fit, F exits to Stretch)
test('pressing F key toggles to Stretch mode', async ({ page }) => {
  await loadImage(page)
  await page.keyboard.press('f')
  await expect(page.getByRole('button', { name: 'Fit mode' })).toHaveAttribute('aria-pressed', 'false')
})

// AC-1: pressing F twice returns to Fit mode
test('pressing F twice returns to Fit mode', async ({ page }) => {
  await loadImage(page)
  await page.keyboard.press('f')
  await page.keyboard.press('f')
  await expect(page.getByRole('button', { name: 'Fit mode' })).toHaveAttribute('aria-pressed', 'true')
...
test('placing a point in Fit mode shows a coordinate label', async ({ page }) => {
  await loadImage(page)
  // Image loads with Fit mode already active
...
test('orientation swap button is re-enabled after switching back to Stretch mode', async ({ page }) => {
  ...
  await fileInput.setInputFiles(LANDSCAPE_IMAGE)
  await page.waitForSelector('img[alt="Reference picture"]')
  // Image loads with Fit mode already active; one click returns to Stretch
